...
/**
 * this class represents the changelog table and it's relations.
 */
@Entity
...
public class ChangelogEntity extends BaseEntity {
     ...
     * <br><br>
     * there is a good reason to use Enums for the changeActions but also enums will eat DB space for no reason, and they are just
     * repetitions
     ...
     * require us to search the DB prior to use them and to use them consistently we will need another enum mapping the DB's content.<br>
     * With
     * a local cache, that pre-fetch could be done on startup with ease, but for the moment we will stick to regular enums to showcase the
     * use
